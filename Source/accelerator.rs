...
//!
//! [`Accelerator`s](crate::accelerator::Accelerator) are used to define a
//! keyboard shortcut consisting of an optional combination of modifier keys
//! (provided by [`Modifiers`]) and one key ([`Code`]).
//!
//! # Examples
	...
	match tokens.len() {
		...
		_ => {
			for raw in tokens {
				...
				if key.is_some() {
					// At this point we have parsed the modifiers and a main
