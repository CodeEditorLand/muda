...
//!
//! - On Windows, accelerators don't work unless the win32 message loop calls [`TranslateAcceleratorW`](https://docs.rs/windows-sys/latest/windows_sys/Win32/UI/WindowsAndMessaging/fn.TranslateAcceleratorW.html).
//!   See [`Menu::init_for_hwnd`](https://docs.rs/muda/latest/x86_64-pc-windows-msvc/muda/struct.Menu.html#method.init_for_hwnd)
//!   for more details
//!
...
//!
//! You can use [`MenuEvent::receiver`] to get a reference to the
//! [`MenuEventReceiver`] which you can use to listen to events when a menu item
//! is activated ```no_run
//! # use muda::MenuEvent;
//! #
...
pub trait ContextMenu {
	...
	///
	/// Returns `true` if menu tracking ended because an item was selected, and
	/// `false` if menu tracking was cancelled for any reason.
	///
	/// # Safety
	///
	...
	///
	/// This can be used along with [`ContextMenu::hpopupmenu`] when
	/// implementing a tray icon menu.
	///
	/// # Safety
	///
	/// The `hwnd` must be a valid window HWND.
	...
	///   cursor position is used.
	///
	/// Returns `true` if menu tracking ended because an item was selected or
	/// clicked outside the menu to dismiss it.
	///
...
impl MenuEvent {
	...
	/// ## Note
	///
	/// This will not receive any events if [`MenuEvent::set_event_handler`] has
	/// been called with a `Some` value.
	pub fn receiver<'a>() -> &'a MenuEventReceiver { &MENU_CHANNEL.1 }
