...
use crate::{ContextMenu, IsMenuItem, MenuId, MenuItemKind, dpi::Position, util::AddOp};
...
impl Menu {
	...
	/// Creates a new menu with given `items`. It calls [`Menu::new`] and
	/// [`Menu::append_items`] internally.
	...
	/// Creates a new menu with the specified id and given `items`. It calls
	/// [`Menu::new`] and [`Menu::append_items`] internally.
	...
	/// Add menu items to the end of this menu. It calls [`Menu::append`] in a
	/// loop internally.
	///
	/// ## Platform-spcific:
	///
	...
	pub fn prepend(&self, item:&dyn IsMenuItem) -> crate::Result<()> {
		self.inner.borrow_mut().add_menu_item(item, AddOp::Insert(0))
	}

	/// Add menu items to the beginning of this menu. It calls
	/// [`Menu::insert_items`] with position of `0` internally.
	///
	/// ## Platform-spcific:
	///
	/// - **macOS:** Only [`Submenu`] can be added to the menu
	...
	/// Remove the menu item at the specified position from this menu and
	/// returns it.
	...
	pub fn items(&self) -> Vec<MenuItemKind> { self.inner.borrow().items() }

	/// Adds this menu to a [`gtk::Window`]
	///
	/// - `container`: this is an optional paramter to specify a container for
	///   the [`gtk::MenuBar`], it is highly recommended to pass a container,
	///   otherwise the menubar will be added directly to the window, which is
	///   usually not the desired behavior. If using a [`gtk::Box`] as a
	///   container, it is added using [`Box::pack_start(menubar, false, false,
	///   0)`](gtk::prelude::BoxExt::pack_start) then reordered to be the first
	///   child of [`gtk::Box`] using [`Box::reorder_child(menubar,
	///   0)`](gtk::prelude::BoxExt::reorder_child).
	///
	/// ## Example:
	/// ```no_run
	...
	where
		...
		self.inner.borrow_mut().init_for_gtk_window(window, container)
	}
	...
	/// with the [`HACCEL`](windows_sys::Win32::UI::WindowsAndMessaging::HACCEL)
	/// returned from [`Menu::haccel`]
	///
	/// #### Example:
	/// ```no_run
	/// # use muda::Menu;
	...
	///
	/// Note that the theme only affects the menu bar itself and not submenus or
	/// context menu.
	///
	/// # Safety
	///
	...
	pub unsafe fn init_for_hwnd_with_theme(
		...
		self.inner.borrow_mut().init_for_hwnd_with_theme(hwnd, theme)
	}
	...
	/// Returns The
	/// [`HACCEL`](windows_sys::Win32::UI::WindowsAndMessaging::HACCEL)
	/// associated with this menu It can be used with
	/// [`TranslateAcceleratorW`](windows_sys::Win32::UI::WindowsAndMessaging::TranslateAcceleratorW)
	/// in the event loop to enable accelerators
	///
	/// The returned
	/// [`HACCEL`](windows_sys::Win32::UI::WindowsAndMessaging::HACCEL) is valid
	/// as long as the [Menu] is.
	#[cfg(target_os = "windows")]
	pub fn haccel(&self) -> isize { self.inner.borrow_mut().haccel() }
	...
	#[cfg(target_os = "macos")]
	pub fn init_for_nsapp(&self) { self.inner.borrow_mut().init_for_nsapp() }
...
impl ContextMenu for Menu {
	...
	unsafe fn show_context_menu_for_nsview(
		...
		self.inner.borrow_mut().show_context_menu_for_nsview(view, position)
	}

	#[cfg(target_os = "macos")]
	fn ns_menu(&self) -> *mut std::ffi::c_void { self.inner.borrow().ns_menu() }
}
