...
impl MenuChild {
	pub fn is_checked(&self) -> bool { self.checked.get() }
	...
	pub fn remove_inner(
		...
		for menus in self.ns_menus.as_ref().unwrap().values() {
			for menu in menus {
				...
				// its children and removing      each child ns menu item
				// that are related to this submenu.
...
impl MenuChild {
	...
	pub fn create_ns_item_for_menu_item(
		...
		unsafe {
			...
			// Store a raw pointer to the `MenuChild` as an instance variable on
			...
			_ => {
				...
				if let PredefinedMenuItemType::About(_) = item_type {
					unsafe {
						...
						// Store a raw pointer to the `MenuChild` as an instance
						// variable on the native menu item
						ns_menu_item.ivars().set(&*self);
					}
				}
