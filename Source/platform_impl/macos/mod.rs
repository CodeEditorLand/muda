...
impl MenuChild {
	...
	pub fn set_accelerator(&mut self, accelerator: Option<Accelerator>) -> crate::Result<()> {
		...
		if let Some(key_equivalent) = key_equivalent {
			let key_equivalent =
				unsafe { NSString::alloc(nil).init_str(key_equivalent.as_str()).autorelease() };
...
impl MenuChild {
	pub fn create_ns_item_for_submenu(&mut self, menu_id: u32) -> crate::Result<id> {
		...
		self.ns_menus.as_mut().unwrap().entry(menu_id).or_default().push(NsMenuRef(id, ns_submenu));

		self.ns_menu_items.entry(menu_id).or_default().push(NsMenuItemRef(ns_menu_item));
	...
	pub fn create_ns_item_for_menu_item(&mut self, menu_id: u32) -> crate::Result<id> {
		let ns_menu_item =
			create_ns_menu_item(&self.text, Some(sel!(fireMenuItemAction:)), &self.accelerator)?;
...
impl dyn IsMenuItem + '_ {
	fn make_ns_item_for_menu(&self, menu_id: u32) -> crate::Result<id> {
		match self.kind() {
			...
			MenuItemKind::Icon(i) => {
				i.inner.borrow_mut().create_ns_item_for_icon_menu_item(menu_id)
			}
		}
	}
}
...
fn make_menu_item_class() -> *const Class {
	...
	INIT.call_once(|| unsafe {
		...
		decl.add_method(sel!(dealloc), dealloc_custom_menuitem as extern fn(&Object, _));

		decl.add_method(
			sel!(fireMenuItemAction:),
			fire_menu_item_click as extern fn(&Object, _, id),
		);
...
extern fn dealloc_custom_menuitem(this: &Object, _: Sel) {
	unsafe {
...
extern fn fire_menu_item_click(this: &Object, _: Sel, _item: id) {
	unsafe {
...
fn create_ns_menu_item(
	...
	unsafe {
		...
		let key_equivalent =
			(*accelerator).map(|accel| accel.key_equivalent()).transpose()?.unwrap_or_default();
...
unsafe fn show_context_menu(ns_menu: id, view: *const c_void, position: Option<Position>) {
	unsafe {
		...
		let (location, in_view) = if let Some(pos) = position.map(|p| p.to_logical(scale_factor)) {
			...
			let pos = LogicalPosition { x: mouse_location.x, y: mouse_location.y };
