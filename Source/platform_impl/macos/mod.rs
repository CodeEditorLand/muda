...
impl MenuChild {
	...
	pub fn create_ns_item_for_menu_item(
		...
		unsafe {
			...
			// Store a raw pointer to the `MenuChild` as an instance variable on the native
			// menu item
	...
	pub fn create_ns_item_for_predefined_menu_item(
			...
			_ => {
				...
				if let PredefinedMenuItemType::About(_) = item_type {
					unsafe {
						...
						// Store a raw pointer to the `MenuChild` as an instance variable on the
						// native menu item
						ns_menu_item.ivars().set(&*self);
					}
				}
		...
		unsafe {
			...
			if let PredefinedMenuItemType::Services = item_type {
				// we have to assign an empty menu as the app's services menu, and macOS will
				// populate it
