...
impl Menu {
	...
	pub fn remove_for_hwnd(&mut self, hwnd: isize) -> crate::Result<()> {
		self.hwnds.remove(&hwnd).ok_or(crate::Error::NotInitialized)?;
	...
	pub fn is_visible_on_hwnd(&self, hwnd: isize) -> bool {
		self.hwnds.get(&hwnd).map(|_| !unsafe { GetMenu(hwnd as _) }.is_null()).unwrap_or(false)
	}
...
impl MenuChild {
	...
	pub fn add_menu_item(&mut self, item: &dyn IsMenuItem, op: AddOp) -> crate::Result<()> {
		...
		{
			child.borrow_mut().root_menu_haccel_stores.extend(self.root_menu_haccel_stores.clone());
		}
	...
	pub fn items(&self) -> Vec<MenuItemKind> {
		self.children.as_ref().unwrap().iter().map(|c| c.borrow().kind(c.clone())).collect()
	}
...
unsafe extern "system" fn menu_subclass_proc(
	...
	match msg {
		...
		WM_UAHDRAWMENUITEM | WM_UAHDRAWMENU if uidsubclass == MENU_SUBCLASS_ID => {
			let menu = obj_from_dwrefdata::<Menu>(dwrefdata);
			let theme = menu.hwnds.get(&(hwnd as _)).copied().unwrap_or(MenuTheme::Auto);
