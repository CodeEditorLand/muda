...
use util::{Accel, decode_wide, encode_wide};
...
impl Menu {
	...
	pub fn add_menu_item(&mut self, item:&dyn IsMenuItem, op:AddOp) -> crate::Result<()> {
...

		{
			...
			child_.parents_hemnu.push((self.hmenu, Some(self.hwnds.clone())));
	...
	pub fn haccel(&self) -> isize { self.haccel_store.borrow().0 as _ }

	pub fn hpopupmenu(&self) -> isize { self.hpopupmenu as _ }
	...
	pub unsafe fn remove_for_hwnd(&mut self, hwnd:isize) -> crate::Result<()> {
		self.hwnds.borrow_mut().remove(&hwnd).ok_or(crate::Error::NotInitialized)?;
...
/// Shared methods
impl MenuChild {
	pub fn item_type(&self) -> MenuItemType { self.item_type }
...
impl MenuChild {
	pub fn set_icon(&mut self, icon:Option<Icon>) {
		...
		let hbitmap = icon.map(|i| unsafe { i.inner.to_hbitmap() }).unwrap_or(std::ptr::null_mut());
...
unsafe fn show_context_menu(
	...
	let result = unsafe {
		let pt = if let Some(pos) = position {
			...
			let mut pt = POINT { x:pos.x as _, y:pos.y as _ };
		...
		TrackPopupMenu(hmenu, TPM_LEFTALIGN | TPM_RETURNCMD, pt.x, pt.y, 0, hwnd, std::ptr::null())
	};
...
fn dwrefdata_from_obj<T>(obj:&T) -> usize { (obj as *const T) as usize }
...
unsafe extern "system" fn menu_subclass_proc(
	...
	match msg {
		...
		WM_UAHDRAWMENUITEM | WM_UAHDRAWMENU if uidsubclass == MENU_SUBCLASS_ID => {
			...
			let theme = menu.hwnds.borrow().get(&(hwnd as _)).copied().unwrap_or(MenuTheme::Auto);
...
unsafe fn menu_selected(hwnd:windows_sys::Win32::Foundation::HWND, item:&mut MenuChild) {
	...
	if dispatch {
		MenuEvent::send(MenuEvent { id:menu_id.unwrap() });
	}
}
...
fn show_about_dialog(hwnd:Hwnd, metadata:&AboutMetadata) {
	...
	let title = encode_wide(format!("About {}", metadata.name.as_deref().unwrap_or_default()));

	#[cfg(not(feature = "common-controls-v6"))]
	std::thread::spawn(move || unsafe {
		use windows_sys::Win32::UI::WindowsAndMessaging::{MB_ICONINFORMATION, MessageBoxW};

		MessageBoxW(hwnd as _, message.as_ptr(), title.as_ptr(), MB_ICONINFORMATION);
	});
	...
	#[cfg(feature = "common-controls-v6")]
	{
		...
		std::thread::spawn(move || unsafe {
			let task_dialog_config = TASKDIALOGCONFIG {
				...
				Anonymous1:TASKDIALOGCONFIG_0 { pszMainIcon:TD_INFORMATION_ICON },
				Anonymous2:TASKDIALOGCONFIG_1 { pszFooterIcon:std::ptr::null() },
