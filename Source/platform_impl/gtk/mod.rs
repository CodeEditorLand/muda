...
impl Menu {
	...
	where
		W: IsA<gtk::Window>,
	{
		self.gtk_menubars.get(&(window.as_ptr() as u32)).map(|m| m.get_visible()).unwrap_or(false)
	}
	...
	fn create_gtk_item_for_predefined_menu_item(
		...
		let item = match predefined_item_type {
			...
			| PredefinedMenuItemType::SelectAll => {
				...
				item.child().unwrap().downcast::<gtk::AccelLabel>().unwrap().set_accel(key, mods);
				item.connect_activate(move |_| {
					// TODO: wayland
					#[cfg(feature = "libxdo")]
...
impl dyn IsMenuItem + '_ {
	fn make_gtk_menu_item(
		...
		self.kind().make_gtk_menu_item(menu_id, accel_group, add_to_cache)
	}
}
