...
pub struct Menu {
	...
	gtk_menu:(u32, Option<gtk::Menu>), /* dedicated menu for tray or context
	                                    * menus */
}
...
impl Menu {
	...
	fn remove_inner(
		...
		for (menu_id, menu_bar) in &self.gtk_menubars {
			...
			//      which is usually when this is the item the user is actaully
			// removing or if we are removing from a specific menu (id)
			...
			if id.map(|i| i == *menu_id).unwrap_or(true) {
				// bail this is not a supported item like a close_window
...
pub struct MenuChild {
	...
	gtk_menu:Option<(u32, Option<gtk::Menu>)>, /* dedicated menu for tray or
	                                            * context menus */
	accel_group:Option<gtk::AccelGroup>,
}
...
/// Shared methods
impl MenuChild {
	pub(crate) fn item_type(&self) -> MenuItemType { self.item_type }
...
impl MenuChild {
	...
	pub fn set_checked(&mut self, checked:bool) {
		self.checked.as_ref().unwrap().store(checked, Ordering::Release);
	...
	pub fn add_menu_item(&mut self, item:&dyn crate::IsMenuItem, op:AddOp) -> crate::Result<()> {
		...
		match op {
			...
			AddOp::Insert(position) => {
				self.children.as_mut().unwrap().insert(position, item.child())
			},
		}
	...
	fn remove_inner(
		...
		for menus in self.gtk_menus.as_ref().unwrap().values() {
			for (menu_id, menu) in menus {
				...
				//      which is usually when this is the item the user is
				// actaully removing or if we are removing from a specific
				// menu (id)      which is when the actual item being
				// removed is a submenu      and we are iterating through
				// its children and removing      each child gtk items that
				// are related to this submenu.
				if id.map(|i| i == *menu_id).unwrap_or(true) {
					// bail this is not a supported item like a close_window
					// predefined menu item
					if is_item_supported!(item) {
