...
impl IsMenuItem for Submenu {
	fn kind(&self) -> MenuItemKind { MenuItemKind::Submenu(self.clone()) }
...
impl Submenu {
	...
	/// Creates a new submenu with given `items`. It calls [`Submenu::new`] and
	/// [`Submenu::append_items`] internally.
	...
	/// Creates a new submenu with the specified id and given `items`. It calls
	/// [`Submenu::new`] and [`Submenu::append_items`] internally.
	...
	/// It calls [`Menu::prepend`](crate::Menu::prepend) on the first element
	/// and passes the rest to
	/// [`Menu::insert_items`](crate::Menu::insert_items) with position of `1`.
	...
	/// Remove the menu item at the specified position from this submenu and
	/// returns it.
	...
	///
	/// If no menu is set as the Help menu, macOS will automatically use any
	/// menu which has a title matching the localized word "Help".
	#[cfg(target_os = "macos")]
...
impl ContextMenu for Submenu {
	...
	#[cfg(target_os = "linux")]
	fn show_context_menu_for_gtk_window(&self, w:&gtk::Window, position:Option<Position>) -> bool {
